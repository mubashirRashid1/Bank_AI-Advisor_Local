...
def search_whitelisted_sources(query, limit=5):
    ...
    Called automatically when Snowflake has insufficient data.
    """
    try:
        import requests
        
        # Build domain filter for Tavily
        domain_filter = " OR ".join([
            f"site:{domain}" 
            for domain in WHITELISTED_SOURCES
        ])
        ...
        # Save each article to Snowflake and embed
        saved = 0
...
def semantic_search_news(query, limit=8):
    """
    Semantic search with automatic whitelisted fallback
    if insufficient results found in Snowflake.
    """
    MINIMUM_RESULTS = 3   # threshold to trigger fallback

    try:
        # ── Step 1 — Try Snowflake semantic search ────────────────
        conn   = get_snowflake_connection()
        cursor = conn.cursor()
        ...
        # ── Step 2 — Check if enough results ─────────────────────
#        if len(news_df) >= MINIMUM_RESULTS:
 #           return news_df, "snowflake"   # ← return source too
        # REPLACE WITH THIS
        MINIMUM_RESULTS      = 3
        MINIMUM_RELEVANCE    = 0.5   # articles must actually be relevant

        if not news_df.empty and len(news_df) >= MINIMUM_RESULTS:
            ...
            if 'RELEVANCE_SCORE' in news_df.columns:
                avg_relevance = pd.to_numeric(
                    news_df['RELEVANCE_SCORE'], errors='coerce'
                ).mean()

                if avg_relevance >= MINIMUM_RELEVANCE:
                    return news_df, "snowflake"
                # Low relevance — fall through to whitelist
            else:
                # Cortex Search doesn't return relevance scores
                # Trust the semantic ranking — return as is
                return news_df, "snowflake"
                
        # ── Step 3 — Not enough — trigger whitelisted fallback ────
        saved, found = search_whitelisted_sources(query, limit=5)

        if saved > 0:
            # Re-run semantic search with fresh data
            # Wait briefly for Cortex Search to index
            import time
            time.sleep(2)
        ...
        return news_df, "whitelisted_fallback"
